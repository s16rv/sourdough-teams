...
import { Secp256k1Verifier, VerifySignatureManual } from "../../typechain-types";

describe("Secp256k1Verifier", function () {
    let verifySignatureManual: VerifySignatureManual;
    let secp256k1Verifier: Secp256k1Verifier;
    beforeEach(async function () {
        const VerifySignatureManualContract = await hre.ethers.getContractFactory("VerifySignatureManual");
        verifySignatureManual = await VerifySignatureManualContract.deploy();
        await verifySignatureManual.waitForDeployment();
    ...
    it("Should verify signature generated", async function () {
        ...
        const signer = await verifySignatureManual.verifySignature(
